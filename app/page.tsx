  ...
  const onFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (ev: ProgressEvent<FileReader>) => {
      try {
        ...
        if (err instanceof Error) setStatus("Erro ao processar arquivo: " + err.message);
        else setStatus("Erro inesperado ao processar arquivo.");
      }
    };
  ...
  return (
    <main className="min-h-screen bg-slate-50 text-slate-900">
      <div className="max-w-6xl mx-auto p-6 space-y-6">
        ...
        <Card title="Ranking por Usuário — Unidades x Caixas">
          <div className="h-[360px] md:h-[420px]">
            <Bar data={barData} options={barOptions()} plugins={[ChartDataLabels]} />
          </div>
        </Card>

        <Card title="Evolução por Hora — Unidades x Caixas">
          <div className="h-[360px] md:h-[420px]">
  ...
  return {
    ...
    plugins: {
      ...
      datalabels: {
        ...
        borderRadius: 4,
        padding: 3,
        formatter: (v: number) => (v ? v.toLocaleString("pt-BR") : ""),
      },
    },
    scales: {
